...
def sentence1():
    ...
    not_b = ~b
    a_or_b = a | b
    if_and_only = not_a % (not_b | c)
    not_a_ornot_b_or_c = disjoin(~a, ~b, c)
    return conjoin([a_or_b, if_and_only, not_a_ornot_b_or_c])
...
def sentence3():
    ...
    C = PropSymbolExpr("PacmanAlive", 0)
    D = PropSymbolExpr("PacmanAlive", 0)
    sentence1 = B % disjoin(conjoin(A, ~D), conjoin(~A, C))
    sentence2 = ~conjoin(A, C)
    sentence3 = C
